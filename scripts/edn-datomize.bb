...
;; kotoba-lang/occupation 用に com-junkawasaki/root superproject の
;; manifest/edn-datomize.bb から移植（Phase 3 fanout, gftdcojp/net-kotobase の
;; Phase 2 pilot 版を経由）。schema-path をこの repo のルート schema.edn に
;; 向けている以外はロジック同一 ＋ wrap-generic モード（既に名前空間付きキーを
;; 持つ map 用）。
;;
...
;;  読める）。属性定義はこの repo ルートの schema.edn に自動登録する（Datomic/
;; Datascript 両対応、:db.install/_attribute 等の Datomic 固有キーは使わない）。
;;
;; 使い方:
;;   bb scripts/edn-datomize.bb wrap-map     <path> <ns>         — map 1個のファイルを変換
;;   bb scripts/edn-datomize.bb wrap-generic <path> <default-ns> — 既に名前空間付きキーを持つ map を変換
...
;; occupation (child repo) has no manifest/ dir — schema lives at repo root.
